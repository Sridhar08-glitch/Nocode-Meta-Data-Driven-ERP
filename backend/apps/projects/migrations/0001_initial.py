# Generated by Django 5.2.15 on 2026-06-24 09:18

import uuid

import django.utils.timezone
from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = []

    operations = [
        migrations.CreateModel(
            name="ProjectBaseline",
            fields=[
                (
                    "id",
                    models.UUIDField(
                        default=uuid.uuid4,
                        editable=False,
                        primary_key=True,
                        serialize=False,
                    ),
                ),
                (
                    "created_at",
                    models.DateTimeField(
                        db_index=True, default=django.utils.timezone.now
                    ),
                ),
                ("updated_at", models.DateTimeField(auto_now=True)),
                (
                    "deleted_at",
                    models.DateTimeField(blank=True, db_index=True, null=True),
                ),
                ("deleted_by", models.UUIDField(blank=True, null=True)),
                ("created_by", models.UUIDField(blank=True, null=True)),
                ("updated_by", models.UUIDField(blank=True, null=True)),
                ("workspace_id", models.UUIDField(db_index=True)),
                ("project_record_id", models.UUIDField(db_index=True)),
                ("baseline_date", models.DateField(blank=True, null=True)),
                (
                    "planned_budget",
                    models.DecimalField(decimal_places=2, default=0, max_digits=16),
                ),
                ("planned_end_date", models.DateField(blank=True, null=True)),
                ("snapshot", models.JSONField(default=dict)),
            ],
            options={
                "db_table": "project_baselines",
                "indexes": [
                    models.Index(
                        fields=["workspace_id", "project_record_id", "-baseline_date"],
                        name="project_bas_workspa_b730af_idx",
                    )
                ],
            },
        ),
        migrations.CreateModel(
            name="ProjectCostEntry",
            fields=[
                (
                    "id",
                    models.UUIDField(
                        default=uuid.uuid4,
                        editable=False,
                        primary_key=True,
                        serialize=False,
                    ),
                ),
                (
                    "created_at",
                    models.DateTimeField(
                        db_index=True, default=django.utils.timezone.now
                    ),
                ),
                ("updated_at", models.DateTimeField(auto_now=True)),
                (
                    "deleted_at",
                    models.DateTimeField(blank=True, db_index=True, null=True),
                ),
                ("deleted_by", models.UUIDField(blank=True, null=True)),
                ("created_by", models.UUIDField(blank=True, null=True)),
                ("updated_by", models.UUIDField(blank=True, null=True)),
                ("workspace_id", models.UUIDField(db_index=True)),
                ("project_record_id", models.UUIDField(db_index=True)),
                ("source", models.CharField(default="other", max_length=20)),
                ("description", models.CharField(blank=True, max_length=255)),
                (
                    "amount",
                    models.DecimalField(decimal_places=2, default=0, max_digits=16),
                ),
                ("entry_date", models.DateField(blank=True, null=True)),
                ("reference", models.CharField(blank=True, max_length=128)),
                ("posted", models.BooleanField(default=True)),
                ("journal_entry_id", models.UUIDField(blank=True, null=True)),
            ],
            options={
                "db_table": "project_cost_entries",
                "indexes": [
                    models.Index(
                        fields=["workspace_id", "project_record_id", "source"],
                        name="project_cos_workspa_a5e678_idx",
                    )
                ],
            },
        ),
    ]
