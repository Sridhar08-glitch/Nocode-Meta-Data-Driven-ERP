# Generated by Django 5.2.15 on 2026-07-01 05:40

import uuid

import django.utils.timezone
from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = []

    operations = [
        migrations.CreateModel(
            name="Installment",
            fields=[
                (
                    "id",
                    models.UUIDField(
                        default=uuid.uuid4,
                        editable=False,
                        primary_key=True,
                        serialize=False,
                    ),
                ),
                (
                    "created_at",
                    models.DateTimeField(
                        db_index=True, default=django.utils.timezone.now
                    ),
                ),
                ("updated_at", models.DateTimeField(auto_now=True)),
                (
                    "deleted_at",
                    models.DateTimeField(blank=True, db_index=True, null=True),
                ),
                ("deleted_by", models.UUIDField(blank=True, null=True)),
                ("created_by", models.UUIDField(blank=True, null=True)),
                ("updated_by", models.UUIDField(blank=True, null=True)),
                ("workspace_id", models.UUIDField(db_index=True)),
                ("plan_id", models.UUIDField(db_index=True)),
                ("seq", models.PositiveIntegerField(default=1)),
                ("due_date", models.DateField()),
                (
                    "amount",
                    models.DecimalField(decimal_places=2, default=0, max_digits=16),
                ),
                (
                    "paid_amount",
                    models.DecimalField(decimal_places=2, default=0, max_digits=16),
                ),
                (
                    "status",
                    models.CharField(
                        choices=[
                            ("pending", "Pending"),
                            ("partial", "Partial"),
                            ("paid", "Paid"),
                            ("overdue", "Overdue"),
                            ("waived", "Waived"),
                        ],
                        default="pending",
                        max_length=8,
                    ),
                ),
                ("paid_at", models.DateTimeField(blank=True, null=True)),
                (
                    "late_fee_amount",
                    models.DecimalField(decimal_places=2, default=0, max_digits=16),
                ),
                ("late_fee_charged", models.BooleanField(default=False)),
                ("late_fee_journal_entry_id", models.UUIDField(blank=True, null=True)),
                ("reminder_sent_at", models.DateTimeField(blank=True, null=True)),
                ("dunning_level", models.PositiveIntegerField(default=0)),
            ],
            options={
                "db_table": "collection_installments",
                "indexes": [
                    models.Index(
                        fields=["workspace_id", "plan_id", "seq"],
                        name="collection__workspa_234f38_idx",
                    ),
                    models.Index(
                        fields=["workspace_id", "status", "due_date"],
                        name="collection__workspa_a85ebb_idx",
                    ),
                ],
                "unique_together": {("workspace_id", "plan_id", "seq")},
            },
        ),
        migrations.CreateModel(
            name="InstallmentPlan",
            fields=[
                (
                    "id",
                    models.UUIDField(
                        default=uuid.uuid4,
                        editable=False,
                        primary_key=True,
                        serialize=False,
                    ),
                ),
                (
                    "created_at",
                    models.DateTimeField(
                        db_index=True, default=django.utils.timezone.now
                    ),
                ),
                ("updated_at", models.DateTimeField(auto_now=True)),
                (
                    "deleted_at",
                    models.DateTimeField(blank=True, db_index=True, null=True),
                ),
                ("deleted_by", models.UUIDField(blank=True, null=True)),
                ("updated_by", models.UUIDField(blank=True, null=True)),
                ("workspace_id", models.UUIDField(db_index=True)),
                ("number", models.CharField(blank=True, max_length=64)),
                ("subject_ref", models.CharField(blank=True, max_length=64)),
                ("source_document_ref", models.CharField(blank=True, max_length=64)),
                ("notify_ref", models.CharField(blank=True, max_length=64)),
                (
                    "total_amount",
                    models.DecimalField(decimal_places=2, default=0, max_digits=16),
                ),
                ("currency", models.CharField(blank=True, max_length=3)),
                ("num_installments", models.PositiveIntegerField(default=1)),
                (
                    "frequency",
                    models.CharField(
                        choices=[
                            ("weekly", "Weekly"),
                            ("monthly", "Monthly"),
                            ("quarterly", "Quarterly"),
                            ("yearly", "Yearly"),
                            ("custom", "Custom (interval_days)"),
                        ],
                        default="monthly",
                        max_length=12,
                    ),
                ),
                ("interval_days", models.PositiveIntegerField(default=0)),
                ("start_date", models.DateField()),
                ("grace_days", models.PositiveIntegerField(default=0)),
                (
                    "late_fee_type",
                    models.CharField(
                        choices=[
                            ("none", "None"),
                            ("flat", "Flat amount"),
                            ("percent", "Percent of outstanding"),
                        ],
                        default="none",
                        max_length=8,
                    ),
                ),
                (
                    "late_fee_value",
                    models.DecimalField(decimal_places=2, default=0, max_digits=16),
                ),
                ("receivable_account", models.CharField(blank=True, max_length=32)),
                (
                    "late_fee_income_account",
                    models.CharField(blank=True, max_length=32),
                ),
                (
                    "status",
                    models.CharField(
                        choices=[
                            ("active", "Active"),
                            ("completed", "Completed"),
                            ("cancelled", "Cancelled"),
                        ],
                        default="active",
                        max_length=10,
                    ),
                ),
                (
                    "external_ref",
                    models.CharField(blank=True, db_index=True, max_length=128),
                ),
                ("created_by", models.UUIDField(blank=True, null=True)),
            ],
            options={
                "db_table": "collection_plans",
                "indexes": [
                    models.Index(
                        fields=["workspace_id", "status"],
                        name="collection__workspa_c81c44_idx",
                    ),
                    models.Index(
                        fields=["workspace_id", "subject_ref"],
                        name="collection__workspa_cef849_idx",
                    ),
                    models.Index(
                        fields=["workspace_id", "source_document_ref"],
                        name="collection__workspa_6c8da1_idx",
                    ),
                    models.Index(
                        fields=["workspace_id", "external_ref"],
                        name="collection__workspa_cfef84_idx",
                    ),
                ],
            },
        ),
    ]
